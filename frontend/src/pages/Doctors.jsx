  ...
  return (
    <div className="">
      ...
      <div className="flex flex-col sm:flex-row gap-5 mt-5 items-start">
        ...
        <div className="w-full grid grid-cols-[repeat(auto-fit,minmax(240px,max-content))]  gap-10 gap-y-6">
          {filterDoc.map((item, index) => (
            <div

              onClick={() => item.available && navigate(`/appointment/${item._id}`)}
              className={`max-w-70 max-h-100 border border-[#C9D8FF] rounded-xl overflow-hidden ${item.available?'cursor-pointer hover:translate-y-[-10px] transition-all duration-500 ':'cursor-not-allowed'}`}
              key={index}
            >
              <img className="bg-blue-50 h-auto object-cover" src={item.image} alt={item.name} />
              <div className="p-4">
                {item.available ? <div className='flex items-center gap-2 text-sm text-center text-green-500'>
                  <p className='w-2 h-2 bg-green-500 rounded-full'></p><p>Available</p>
                </div> : <div className='flex items-center gap-2 text-sm text-center text-gray-500'>
                  <p className='w-2 h-2 bg-gray-500 rounded-full'></p><p>Unavailable</p>
                </div>}
